-- ============================================================================
-- حقل حساب فيسبوك (اختياري) — يُعامَل كوسيلة اتصال إضافية بنفس نمط الهاتف/
-- الواتساب/البريد: تفضيل خصوصية خاص به (show_facebook)، ولا يظهر في الدليل
-- العام إلا إذا وافق صاحب الملف صراحة.
-- ============================================================================

alter table public.contacts add column if not exists facebook text;

alter table public.publication_preferences
...
select
  ...
  case when pp.show_facebook then c.facebook else null end as facebook
