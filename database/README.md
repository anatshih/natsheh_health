...
- **`016_staff_accounts.sql` (`staff_name`, `staff_email` على `app_users`):** حسابات الفريق (مدير/مراجع) لا تملك سجل `identities` (مخصص لأصحاب الملفات المسجَّلين في الدليل فقط)، ولا يمكن قراءة بريدها الحقيقي من `auth.users` بدون مفتاح `service_role` — لذا خُزِّن الاسم والبريد هنا فقط لغرض العرض في شاشة "المستخدمون" (`app/admin/users`). لا سياسات RLS إضافية مطلوبة؛ سياسة "الإدارة تدير الحسابات" الحالية تغطيها.
