-- ============================================================================
-- حقل "نوع مكان العمل" (مستشفى/عيادة/صيدلية/مختبر/إلخ) — قائمة ثابتة في كود
-- التطبيق وليست جدولاً مُدارًا، لأنها فئات عامة قليلة التغيّر لا تحتاج واجهة
-- إدارة مخصصة كالتخصصات والفروع. يُعرض في الدليل العام إن سمح صاحبه بعرض
-- جهة العمل (نفس تفضيل show_employer).
-- ============================================================================

alter table public.professional_profiles add column if not exists workplace_type text;
...
-- ملاحظة: العمود الجديد أُضيف في نهاية قائمة أعمدة الـ View عمدًا — Postgres
-- يرفض CREATE OR REPLACE VIEW إن غيّر موضع/اسم عمود قائم، فالإضافة تكون دائمًا
-- في آخر القائمة.
