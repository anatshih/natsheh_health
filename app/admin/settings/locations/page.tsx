              ...
              {countries.map((c) => (
                <div key={c.id} className="flex items-center justify-between border-b border-slate-50 py-2 last:border-0 text-sm">
                  ...
                  {isAdmin && (
                    <form
                      action={async (fd) => {
                        const result = await toggleCountry(fd);
            ...
            <form
              action={async (fd) => {
                ...
                if (result.error) {
                  ...
                  setToast({ message: 'تمت إضافة الدولة.', type: 'success' });
                  load();
                }
              }}
              ...
              {cities.map((c: any) => (
                <div key={c.id} className="flex items-center justify-between border-b border-slate-50 py-2 last:border-0 text-sm">
                  ...
                  {isAdmin && (
                    <form
                      action={async (fd) => {
                        const result = await toggleCity(fd);
                        if (result.error) setToast({ message: result.error, type: 'error' });
                        else load();
                      }}
                    >
            ...
            <form
              action={async (fd) => {
                ...
                if (result.error) {
                  ...
                  setToast({ message: 'تمت إضافة المدينة.', type: 'success' });
                  load();
                }
              }}
              className="mt-3 flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-5"
